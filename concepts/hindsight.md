...
- **sitecustomize.py monkey-patch廃止**: LLM統一によりauto-fallback不要に（5/5）
...
### 既知の問題
- **Ollama Cloud推論障害**: 5/2にOllama Cloudの/chat/completionsエンドポイント全体がtimeout。/v1/modelsは正常。auth通るが推論エンジン不応答。ZAI直通は正常。→ LLMをopencode-go deepseek-v4-flashに統一切替で解決（5/5）
- **429 Rate Limit**: glm-4.7 (Ollama Cloud) で頻発（4/24確認）→ cron jobs遅延・スキップ。opencode-go統一で解決済み
